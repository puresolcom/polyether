...
class TaxonomyController extends Controller
{
    ...
    public function getIndex( $taxName )
    {
        ...
        $dataTable->addColumns( [ [ 'data' => 'name', 'name' => 'name', 'label' => 'Name', ],
    ...
    public function postGetTaxonomyDataTableResult( $taxonomy )
    {
        ...
        $args = [ 'columns' => [ 'terms.id', 'name', 'description', 'slug', 'count', 'parent' ],
        ...
        $dataTables->addColumn( 'actions', function( $term ) use ( $taxonomy ) {
            ...
            if ( Auth::user()->can( 'edit_terms' ) ) {
                $output .= '<a href="' . route( 'term_taxonomy_edit', [ $taxonomy->name,
                                                                        $term->id ] ) . '" class="btn btn-xs btn-primary"><i class="glyphicon glyphicon-edit"></i> Edit</a>';
            }
        ...
        if ( Request::ajax() && Request::has( [ 'term_id', 'taxonomy' ] ) ) {
            ...
            if ( $delete != 0 ) {
                ...
                if ( $taxonomy->hierarchical ) {
                    $response[ 'success' ][ 'replaces' ][ Request::get( 'taxonomy' ) . '_parent' ] = view( 'backend::taxonomy.taxonomy-parent-select', [ 'exclude' => [ (int)Request::get( 'term_id' ) ],
                                                                                                                                                         'taxName' => Request::get( 'taxonomy' ) ] )->render();
                }
            } else {
    ...
    public function putEditTerm( $taxonomyName, $termId )
    {
        ...
        if ( $updateTerm instanceof EtherError ) {
            return redirect( route( 'term_taxonomy_edit', [ $taxonomyName,
                                                            $termId ] ) )->with( 'error', $updateTerm->all() );
        }

        $redirectUrl = route( 'taxonomy_home', $taxonomyName );

        return redirect( route( 'term_taxonomy_edit', [ $taxonomyName,
                                                        $termId ] ) )->with( 'success', [ '<b>Done</b>, Term updated successfully <br /><br />',
