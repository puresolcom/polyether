...
class Backend
{
    ...
    private function _registerCoreAssets()
    {
        Plugin::add_action( 'ether_backend_head', function() {
            ...
            Asset::container( 'backend_header' )
                 ->add( 'select2-css', 'vendor/backend/plugins/select2/select2.min.css' );
            Asset::container( 'backend_header' )->add( 'backend-css', 'vendor/backend/css/backend-adminlte.css' );
            Asset::container( 'backend_header' )
                 ->add( 'font-awesome-css', 'https://maxcdn.bootstrapcdn.com/font-awesome/4.3.0/css/font-awesome.min.css' );
            Asset::container( 'backend_header' )
                 ->add( 'ionicons-css', 'https://code.ionicframework.com/ionicons/2.0.1/css/ionicons.min.css' );
            Asset::container( 'backend_header' )
                 ->add( 'bootstrap-datetimepicker-css', 'vendor/backend/plugins/bootstrap-datetimepicker/css/bootstrap-datetimepicker.min.css' );
        ...
        Plugin::add_action( 'ether_backend_foot', function() {
            ...
            Asset::container( 'backend_footer' )
                 ->add( 'bootstrap-datetimepicker-js', 'vendor/backend/plugins/bootstrap-datetimepicker/js/bootstrap-datetimepicker.min.js', [ 'jquery',
                                                                                                                                               'moment-js' ] );
        }, 2 );
    }
            ...
            if ( ! empty( $postTypes ) ) {
                foreach ( $postTypes as $postType ) {
                    if ( true == $postType->show_in_admin_menu ) {
                        ...
                        $parent_slug = $this->registerMenuPage( $slug, $title, $permissions, $link, $icon, $postType->menu_position );
                            ...
                            if ( ! empty( $taxonomies ) ) {
                                foreach ( $taxonomies as $taxonomy ) {
                                    if ( true == $taxonomy->show_ui && true == $taxonomy->show_in_admin_menu ) {
                                        ...
                                        $icon = '';
                                        $this->registerMenuSubPage( $parent_slug . '_' . $taxonomy->name, $parent_slug, $title, [ ], $link, $icon );
                                    }
    ...
    public function registerMenuPage( $slug, $title, array $permissions, $link = null, $icon = null, $position = null )
    {
        ...
        if ( ! empty( $slug ) && ! empty( $title ) ) {
            $this->menuLinks[ $slug ] = [ 'title' => $title, 'permissions' => $permissions, 'link' => $link,
                                          'icon'  => $icon, 'position' => (int)$position, ];
